...
export default function Dashboard() {
  ...
  const cardBorder = isDarkMode ? 'border-[#5a443c]' : 'border-gray-200';
  const iconBg = isDarkMode ? 'bg-[#3C2A1F]' : 'bg-gray-100';
  const buttonBg = isDarkMode 
    ? 'bg-[#3C2A1F] hover:bg-[#46342e] text-gray-300' 
    : 'bg-gray-200 hover:bg-gray-300 text-gray-800';
  const textColor = isDarkMode ? 'text-gray-400' : 'text-gray-500';
  const headingColor = isDarkMode ? 'text-white' : 'text-gray-900';
  
  // Definindo cores para os gráficos
  const chartGridColor = isDarkMode ? '#5a443c' : '#e5e7eb';
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip 
                  contentStyle={{
                    backgroundColor: isDarkMode ? '#2a211c' : 'white',
                    borderColor: isDarkMode ? '#5a443c' : '#e5e7eb',
